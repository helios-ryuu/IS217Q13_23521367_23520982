...
def process_chunks(input_file: str, output_file: str, chunk_size: int = 100000,
...
def parse_arguments():
  ...
  python preprocess.py input.csv -o output.csv -c 100000
  python preprocess.py data.csv --date-cutoff 2020-01-01 --chunk-size 100000
  python preprocess.py data.csv --delete-columns ID,Country,Description
    ...
    parser.add_argument(
        ...
        type=int,
        default=100000,
        help='Kích thước khối xử lý (mặc định: 500,000 dòng)'
    )
...
def main(input_file: str = "../../../data/US_Accidents_March23.csv",
    ...
    # Nếu không có output_file, tạo file đầu ra cùng thư mục với preprocess.py
    if output_file is None:
        script_dir = os.path.dirname(os.path.abspath(__file__))
        input_basename = os.path.splitext(os.path.basename(input_file))[0]
        output_file = os.path.join(script_dir, f"{input_basename}-preprocessed.csv")
...
if __name__ == "__main__":
    try:
        ...
        if args.output_file is None:
            ...
            args.output_file = os.path.join(script_dir, f"{input_basename}-preprocessed.csv")
